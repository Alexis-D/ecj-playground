...
public class Squares extends Problem implements SimpleProblemForm {
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		epsilon = state.parameters.getDoubleWithMax(base.push(P_EPSILON),
				def.push(P_EPSILON), 0.d, 1.d);

		if (epsilon < 0.d) {
			state.output.fatal("Epsilon should be in the interval [0, 1].",
					base.push(P_EPSILON), def.push(P_EPSILON));
		}
	}
	...
	public void evaluate(final EvolutionState state, final Individual ind,
		...
		((SimpleFitness) iInd.fitness).setFitness(state,
				1.f / (fitness + (float) epsilon), fitness == 0.f);
		iInd.evaluated = true;
	}
}
